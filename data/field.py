...
class ImageDetectionsField(RawField): # 对Object特征处理：load、preprocess、process
...
class CtxField(RawField):
    def __init__(self, ctx_file, k=8, preload=False, preprocessing=None, postprocessing=None):
        self.k = k

        self.preload = preload
        if preload:
            self.ctx = self.load(ctx_file/"haav_ctx.hdf5",ctx_file/"vis_ctx.hdf5")
        else:
            self.txt_ctx = ctx_file/"haav_ctx.hdf5"
            self.vis_ctx = ctx_file/"vis_ctx.hdf5"

        super(CtxField, self).__init__(preprocessing, postprocessing)
    
    def __load(self, txt_ctx,vis_ctx, k):
        # ctx-whole
        ctx_whole_f = txt_ctx['%d_whole' % k][:self.k] # (8,512)
        # ctx-five
        ctx_five_f = txt_ctx['%d_five' % k][:, :self.k] #(5,8,512)
        ctx_five_f = ctx_five_f.reshape((5*self.k,-1)) # *(40,512)

        #visual
        visual_f = vis_ctx[str(k)][()] #(1,512)

        return {
            "whole": {"features": ctx_whole_f},
            "five": {"features": ctx_five_f},
            "visual":{"features": visual_f}
            }
    
    def load(self, ctx_file,vis_file):
        print(f"Preload features from {str(ctx_file)}...")
        ctx_file = h5py.File(ctx_file, "r")
        vis_file = h5py.File(vis_file, "r")
        pool = Pool(128)
        results = {
            k: pool.apply_async(self.__load, args=(ctx_file,vis_file,k))
    ...
    def preprocess(self, x):
        #传入的参数x为整型，即img_id
        if self.preload:
            ctx_whole_f = self.txt_ctx[str(x)]["whole"]["features"]
            ctx_five_f = self.txt_ctx[str(x)]["five"]["features"]
            visual_f = self.vis_ctx[str(x)]["visual"]["features"]
        else:
            txtctx = h5py.File(self.txt_ctx, "r")
            visctx = h5py.File(self.vis_ctx, "r")
            data = self.__load(txtctx, visctx, x)
            ctx_whole_f = data["whole"]["features"]
            ctx_five_f = data["five"]["features"]
            visual_f = data["visual"]["features"]
            txtctx.close()
            visctx.close()

        ctx_whole_f = torch.FloatTensor(ctx_whole_f)
        ctx_five_f = torch.FloatTensor(ctx_five_f)
        visual_f = torch.FloatTensor(visual_f)

        return ctx_whole_f, ctx_five_f,visual_f
            
    def process(self, batch, *args, **kwargs):
        ctx_whole_f, ctx_five_f, visual_f = list(zip(*batch))
        
        ctx_whole_f = torch.stack(ctx_whole_f)
        ctx_five_f = torch.stack(ctx_five_f)
        visual_f = torch.stack(visual_f)

        return {
            "whole": {"embed": ctx_whole_f},
            "five": {"embed": ctx_five_f},
            "visual": {"embed": visual_f},
        }


class GridField(RawField):
    def __init__(self, ctx_file, preload=False, preprocessing=None, postprocessing=None):
        ...
        super(GridField, self).__init__(preprocessing, postprocessing)
    ...
    def preprocess(self, image_id):
        if self.preload:
            ...
            ctx = self.ctx['%d_features' % image_id][:]
