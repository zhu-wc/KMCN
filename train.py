...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', type=str, default="/data16/zwc2/ctx_features_dataset") #ctx和全局特征的储存位置，二者将在同一个类中被读取。vis文件以img_id为keys;txt文件以{}_whole和{}_five为keys
    parser.add_argument('--annotations', type=str, default="/data16/wxx/ORAT_Collection/Better_than_m2/annotation")#人工标注的GT文件
    parser.add_argument('--obj_file', type=str, default="/data16/wxx/CTX-M2/m2/vinvl.hdf5")#利用 Vinvl 取得的目标特征，直接以img_id作为keys
    parser.add_argument('--grid_file', type=str, default="/data16/wxx/CLIPGrid/CLIP_features.hdf5") # 利用RSTNet101 取得的网格特征，以%d_features作为keys
    ...
    if args.resume_last or args.resume_best:
        ...
        if os.path.exists(fname):
            data = torch.load(fname,map_location={'cuda:1':str(device)})
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if e == 27:
            if current_lr == args.lr_rl: # 即 5e-6
                patience = 0
                grouped_parameters = [
                    {'params': [p for n, p in model.named_parameters() if not \
                                any(nd in n for nd in no_decay)], 'weight_decay': args.wd_rl},
                ...
                optim = AdamW(grouped_parameters, lr=1, eps=1e-8)
                scheduler = LambdaLR(optim, lambda_rl_lr2)
                for i in range(e):
                    scheduler.step()
                print("Reduce Learining Rate to 5e-7")
                if not best_test:
                    data = torch.load(args.save_dir/'ckpt_best_test.pth')
                    torch.set_rng_state(data['torch_rng_state'])
                    torch.cuda.set_rng_state(data['cuda_rng_state'])
                    np.random.set_state(data['numpy_rng_state'])
                    random.setstate(data['random_rng_state'])
                    model.load_state_dict(data['model'], strict=False)
                    print('Resuming from epoch %d, validation loss %f, best cider %f and best test cider %f' % (
